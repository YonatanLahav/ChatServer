...
namespace ChatApi.Controllers
{
    ...
    public class ApiController : Controller
    {
        ...
        public ApiController(Service service, firebaseService fbService)
        {
            _service = service;
        ...
        */
        ...
        public async Task<IActionResult> CreateMessage([Bind("content")] ApiMessage apiMessage, string contactId, string username)
        {
            if (username == null || username == ""
                ...
                return NotFound();
            List<ApiMessage> msgList;
            if (await _service.CreateNewMessage(apiMessage, username, contactId) == true)
            {
                msgList = _service.GetAllMessages(username, contactId);
                return StatusCode(201, msgList.LastOrDefault());
            }
            else
                return NotFound();
        }
...
/*            if (apiInvitation == null || apiInvitation.from == null
        ...
        public async Task<IActionResult> PostTransfer([Bind("from,to,content")] ApiTransfer apiTransfer)
        {
            ...
            if (await _service.CreateTransferMessage(apiMessage, apiTransfer.to, apiTransfer.from))
            {
                if (LoginController.getUsersTokensDic()[apiTransfer.to] != null)
                {
                    await _fbService.SendNotification(LoginController.getUsersTokensDic()[apiTransfer.to]
                        , "New Message From " + apiTransfer.from, apiTransfer.content);
                }
                return StatusCode(201);
            }
            else
